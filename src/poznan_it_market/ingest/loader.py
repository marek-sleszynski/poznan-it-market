...
START_RUN_QUERY = "INSERT INTO raw.ingestion_runs (run_id, started_at, status) VALUES (%s, %s, 'running');"
...
def load_raw_offers(conn: psycopg.Connection, offers: list[dict], source: str, run_id: uuid.UUID, fetched_at: datetime) -> int:
    records = [
        (source, str(o.get("slug") or o.get("id")), Jsonb(o), fetched_at, run_id)
        for o in offers
    ]
...
def log_run_finish(conn: psycopg.Connection, run_id: uuid.UUID, finished_at: datetime, status: str, count: int = 0) -> None:
    with conn.transaction():
...
def run_pipeline() -> None:
    ...
    with get_connection() as conn:
        ...
        try:
            ...
            log_run_finish(conn, run_id, datetime.now(timezone.utc), "success", loaded_count)
            print(f"SUKCES: Załadowano {loaded_count} ofert! Run ID: {run_id}")
        except Exception as e:
            log_run_finish(conn, run_id, datetime.now(timezone.utc), "failed", 0)
            print(f"BŁĄD: {e}")
            raise
